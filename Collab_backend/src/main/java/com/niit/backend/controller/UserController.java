...
*/			userDAO.setOnline(user.getUser_id());
	...
	/* this method is used in accept and reject */
	...
	
	{
		...
		
		{
			...
			user = new UserDetails(); // It does not mean that we are insert// user
			user.setErrorCode("404");
	...
	public ResponseEntity<UserDetails> deleteUser(@PathVariable("id") String user_id)
	
	{
		System.out.println("calling method deleteUser for user id: " + user_id);
		UserDetails user=userDAO.get(user_id);
		if(user==null)
		
		{
			System.out.println("user does not exists with id:" + user_id);
			user=new UserDetails();
